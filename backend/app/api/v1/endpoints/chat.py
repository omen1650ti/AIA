...
async def policy_checker(old_policy_id: uuid.UUID, new_policy_id: uuid.UUID) -> ChatResponse:
    """
    ...
    """
    response = await ChatService.check_policies(old_policy_id, new_policy_id)
    
    if not response:
        raise HTTPException(
            status_code=500,
            detail="Error checking policies"
        )
