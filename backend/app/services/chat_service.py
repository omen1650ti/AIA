...
from app.schemas.chat import ChatRequest, ChatResponse, ConversationHistoryRequest, ConversationHistoryResponse
...
class ChatService:
    ...
    async def check_policies(old_policy_id: uuid.UUID, new_policy_id: uuid.UUID) -> ChatResponse:
        ...
        policy_info_list = []
        old_policy_info = await get_plan_by_id(old_policy_id)
        new_policy_info = await get_plan_by_id(new_policy_id)
        if old_policy_info:
        ...
        try:
            policy_details = "Policy Details:\n" + "\n".join([f"- {info}" for info in policy_info_list])
            response = run_agent(
    ...
    async def process_message(request: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            # Build response with only thread_id and assistant_response
        ...
        try:
            async with AsyncSessionLocal() as session:
                query = select(ConversationMessage).where(
                    ConversationMessage.thread_id == request.thread_id
